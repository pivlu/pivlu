    ...
    @foreach ($item->menu_links as $navbar_link)
        @if (!empty($navbar_link->dropdown))
            <li class="nav-item dropdown {{ $config->tpl_navbar_links_margin ?? null }}">
                <a class="nav-link dropdown-toggle @if ($navbar_link->btn_id) btn btn_{{ $navbar_link->btn_id }} @endif no-decoration nav-btn" href="#" id="navbarDropdown_{{ $navbar_link->label }}" role="button"
            ...
            <li class="nav-item {{ $config->tpl_navbar_links_margin ?? null }}">
                @if ($navbar_link->btn_id)
                    <span class="btn_{{ $navbar_link->btn_id }}">
                        <a @if ($navbar_link->new_tab == 1) target="_blank" @endif class="nav-link btn btn_{{ $navbar_link->btn_id }} no-decoration nav-btn" href="{{ $navbar_link->url ?? null }}"
