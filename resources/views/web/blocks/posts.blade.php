                ...
                @if ($block_extra['show_image'] ?? null)
                    <div class="col-xl-3 col-lg-4 col-md-5 col-12">
                        <a title="{{ $post->title }}" href="{{ $post->url }}">
                            @if ($post->image)
                                <img src="{{ thumb($post->image) }}" class="img-fluid @if ($block_extra['img-shaddow'] ?? null) img-shaddow @endif mb-3" alt="{{ $post->title }}">
                            @endif
                        </a>
                    </div>
                @endif
                    ...
                    <div class="meta light">
                        @if (($block_extra['show_author'] ?? null) != 'no')
                            @if (($block_extra['show_author'] ?? null) == 'name_avatar')
                                <img src="{{ avatar($post->user_id) }}" alt="{{ $post->author->name }}" class="img-fluid rounded-circle float-start me-2" style="max-height: 20px; max-width:20px;">
                            @endif
                            <a href="{{ route('profile', ['username' => $post->author->username]) }}"> {{ $post->author->name }}</a>
                            <span class="me-2"></span>
                        @endif

                        @if (($block_extra['show_date'] ?? null) != 'no')
                            <i class="bi bi-calendar"></i>
                            @if (($block_extra['show_date'] ?? null) == 'date')
                                {{ date_locale($post->created_at, 'datetime') }}
                            @endif
                            @if (($block_extra['show_date'] ?? null) == 'datetime')
                                {{ date_locale($post->created_at, 'datetime') }}
                            @endif
                            <span class="me-2"></span>
                        @endif
            ...
            <div class="{{ $class ?? 'col-12' }}">
                <div class="listing-box mb-4">
                    ...
                    <div class="meta light">
                        ...
                        @if (($block_extra['show_date'] ?? null) != 'no')
                            <i class="bi bi-calendar"></i>
                            @if ($block_extra['show_date'] == 'date')
                                {{ date_locale($post->created_at) }}
                            @endif
                            @if ($block_extra['show_date'] == 'datetime')
                                {{ date_locale($post->created_at, 'datetime') }}
                            @endif
                            <span class="me-2"></span>
                        @endif
