@if (($config->tpl_navbar_logo_align ?? null) == 'text-center mx-auto')
        ...
        <div class="row">
            <div class="logo align-center mt-2 mb-2">
                @if (!($config->tpl_navbar_hide_logo ?? null))
                    ...
                    <a class="" href="{{ $logo_url }}"><img @if (($config->tpl_navbar_logo_align ?? null) == 'text-center mx-auto') class="float-start" @endif alt="{{ $config_lang->site_label ?? 'Clevada' }}"
                            src="{{ asset('uploads/' . $config->logo) }}"></a>
                @endif

                @if ($config->tpl_navbar_show_custom_text ?? null)
                    <div class="float-end ms-3">
                        {!! nl2br($config->tpl_navbar_custom_text) ?? null !!}
                    </div>
                @endif
            </div>
        </div>
...
@else
    <div class="">
        <div class="container-xxl">
            <div class="row">
                <div class="mt-2 mb-2 {{ $config->tpl_navbar_logo_align ?? 'text-center' }}" @if (($config->tpl_navbar_logo_align ?? null) == 'text-center mx-auto') style="width: auto; max-width:40%;" @endif>
                    @if (!($config->tpl_navbar_hide_logo ?? null))
                        @php
                            if (get_default_language()->code == $locale) {
                                $logo_url = route('home');
                        ...
                        <a class="{{ $config->tpl_navbar_logo_align ?? null }}" href="{{ $logo_url }}"><img alt="{{ $config_lang->site_label ?? 'Clevada' }}"
                                src="{{ asset('uploads/' . $config->logo) }}"></a>
                    @endif
                    ...
                    @if ($config->tpl_navbar_show_custom_text ?? null)
                        @if (($config->tpl_navbar_logo_align ?? null) == 'float-start')
                            <div class="d-block float-end">
                                {!! nl2br($config->tpl_navbar_custom_text) ?? null !!}
                            </div>
                        @endif

                        @if (($config->tpl_navbar_logo_align ?? null) == 'float-end')
...
<nav class="navbar navbar-expand-md @if ($config->tpl_navbar_sticky ?? null) sticky-top @endif {{ $config->tpl_navbar_links_align ?? null }} style_nav">
    <div class="container-xxl">
        ...
        <div class="collapse navbar-collapse" id="navbar1">
            <ul class="navbar-nav {{ $config->tpl_navbar_links_align ?? 'me-auto' }}">
