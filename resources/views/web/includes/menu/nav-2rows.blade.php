<div class="text-center">
    <div class="container-xxl">
        <div class="logo text-center mt-2 mb-2">
            @if (!($config->tpl_navbar_hide_logo ?? null))
                <a href="{{ route('home') }}"><img alt="{{ site()->label }}" src="{{ image($config->logo ?? null) }}"></a>
            @endif
        </div>
...
<nav class="navbar navbar-expand-md @if ($config->tpl_navbar_sticky ?? null) sticky-top @endif {{ get_style('nav') }}">
    <div class="container-xxl">
                ...
                @foreach (menu_links() as $navbar_link)
                    ...
                    @else
                        <li class="nav-item {{ $config->tpl_navbar_links_margin ?? null }}">
                            <a @if ($navbar_link->new_tab == 1) target="_blank" @endif class="nav-link @if ($navbar_link->btn_id) btn btn_{{ $navbar_link->btn_id }} {{ button($navbar_link->btn_id)->font_weight ?? null }} {{ button($navbar_link->btn_id)->rounded ?? null }} {{ button($navbar_link->btn_id)->shadow ?? null }} {{ button($navbar_link->btn_id)->size ?? null }} @endif" href="{{ $navbar_link->url }}"
                                title="{{ site()->label }}">
                                @if ($navbar_link->icon)
                                    {!! $navbar_link->icon !!}
