                ...
                <div class="listing mb-5">
                    ...
                    <div class="meta">
                        @if (($config->tpl_posts_show_author ?? null) == 'name')
                            <div class="post-author clearfix mb-1"><a href="{{ route('profile', ['username' => $post->author->username, 'lang' => getRouteLang()]) }}">
                                    {{ $post->author->name }}</a></div>
                        @elseif (($config->tpl_posts_show_author ?? null) == 'name_avatar')
                            <div class="post-author clearfix mb-1"><img style="height:20px; width: 20px;" src="{{ avatar($post->author->id) }}" alt="{{ $post->author->name }}"
        ...
        <div class="listing mb-5">
            <div class="row">
                ...
                <div class="@if (!($config->tpl_posts_hide_image ?? null)) col-xl-9 col-lg-7 col-md-7 @endif col-12">
                    ...
                    <div class="meta">
                        @if (($config->tpl_posts_show_author ?? null) == 'name')
                            <span class="me-2 post-author"><a href="{{ route('profile', ['username' => $post->author->username, 'lang' => getRouteLang()]) }}">
                                    {{ $post->author->name }}</a></span>
                        @elseif (($config->tpl_posts_show_author ?? null) == 'name_avatar')
                            <span class="me-2 post-author"><img style="height:20px; width: 20px;" src="{{ avatar($post->user_id) }}" alt="{{ $post->author->name ?? null }}"
                                    class="img-fluid rounded-circle float-start me-1">
                                <a href="{{ route('profile', ['username' => $post->author->username, 'lang' => getRouteLang()]) }}"> {{ $post->author->name }}</a>
                            </span>
                        @endif
