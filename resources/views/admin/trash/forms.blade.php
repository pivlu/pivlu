        ...
        <div class="col-12">
            <nav aria-label="breadcrumb" class="breadcrumb-header">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item" aria-current="page"><a href="{{ route('admin.trash') }}">{{ __('Trash') }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{ __('Forms messages') }}</li>
                </ol>
            </nav>
        </div>
    </div>
...
<div class="card">
    ...
    <div class="card-header">
        ...
        <div class="row">
            ...
            <div class="col-12 col-sm-12 col-md-6 order-md-1 order-first">
                <h4 class="card-title">{{ __('Deleted forms messages') }} ({{ $items->total() ?? 0 }} {{ __('items') }})</h4>
            </div>
            ...
            <div class="col-12 col-sm-12 col-md-6 order-md-2 order-last">
                <div class="float-end">
                    ...
                    @include('pivlu::admin.trash.modals.empty-trash')
                </div>
            </div>
    ...
    <div class="card-body">
        ...
        <section>
            <form action="{{ route('admin.trash.module', ['module' => 'forms']) }}" method="get" class="row row-cols-lg-auto g-3 align-items-center">
                ...
                <div class="col-12">
                    <select name="search_status" class="form-select me-2 mb-2 @if ($search_form_id) is-valid @endif">
                        <option value="">- {{ __('All forms') }} -</option>
                        @foreach($forms as $form)
                        <option @if ($search_form_id==$form->id) selected @endif value="{{ $form->id }}">{{ $form->label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-12">
                    <select class="form-select me-2 mb-2 @if ($search_replied) is-valid @endif" name="search_replied">
                        ...
                        <option @if ($search_replied=='no' ) selected="selected" @endif value="no">{{ __('Messages without reply') }}</option>
                        <option @if ($search_replied=='yes' ) selected="selected" @endif value="yes">{{ __('Messages with reply') }}</option>
                    </select>
                </div>
                ...
                <div class="col-12">
                    <select class="form-select me-2 mb-2 @if ($search_important) is-valid @endif" name="search_important">
                        ...
                        <option @if ($search_important=='1' ) selected="selected" @endif value="1">{{ __('Only important') }}</option>
                    </select>
                </div>

                <div class="col-12">
                    <button class="btn btn-secondary me-2 mb-2" type="submit"><i class="bi bi-check2"></i> {{ __('Apply') }}</button>
                    <a class="btn btn-light  mb-2" href="{{ route('admin.trash.module', ['module' => 'forms']) }}"><i class="bi bi-arrow-counterclockwise"></i></a>
                </div>
        ...
        {{ __('No item') }}
        @else
        <form method="POST" action="{{ route('admin.trash.multiple_action', ['module' => 'forms']) }}">
            @csrf
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="d-grid gap-2">
                                    <a href="{{ route('admin.trash.single_action', ['module' => 'forms', 'id' => $message->id, 'return_to' => 'trash', 'action' => 'delete']) }}"
                                        class="btn btn-danger btn-sm mb-2">{{ __('Permanently delete') }}</a>

                                    <a href="{{ route('admin.trash.single_action', ['module' => 'forms', 'id' => $message->id, 'return_to' => 'trash', 'action' => 'restore']) }}"
            ...
            <div class="row row-cols-lg-auto g-3">
                ...
                <div class="col-12">
                    <input type="hidden" name="return_to" value="trash">
