...
<div class="card">
    ...
    <div class="card-body">
        ...
        <section>
            <form action="{{ route('admin.trash.module', ['module' => 'posts']) }}" method="get" class="row row-cols-lg-auto g-3 align-items-center">
                ...
                <div class="col-12">
                    <select class="form-select me-2 mb-2 @if ($search_post_type_id) is-valid @endif" name="search_post_type_id">
                        <option selected="selected" value="">- {{ __('All post types') }} -</option>
                        @foreach ($post_types as $post_type)
                            <option @if ($post_type->id == $search_post_type_id) selected @endif value="{{ $post_type->id }}">{{ $post_type->default_language_content->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-12">
                    ...
                    <a class="btn btn-light  mb-2" href="{{ route('admin.trash.module', ['module' => 'posts']) }}"><i class="bi bi-arrow-counterclockwise"></i></a>
                </div>
        ...
        @if ($items->total() == 0)
            {{ __('No item') }}
        @else
            <form method="POST" action="{{ route('admin.trash.multiple_action', ['module' => 'posts']) }}">
                @csrf
            ...
            {{ $items->appends(['search_terms' => $search_terms, 'search_post_type_id' => $search_post_type_id])->links() }}
