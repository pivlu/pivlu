<nav class="nav nav-tabs" id="myTab" role="tablist">
    ...
    <a class="nav-item nav-link @if (($theme_tab ?? null) == 'style') active @endif" href="{{ route('admin.themes.show', ['id' => $theme->id, 'theme_tab' => 'style']) }}">{{ __('Style') }}</a>
    <a class="nav-item nav-link @if (($theme_tab ?? null) == 'nav') active @endif" href="{{ route('admin.themes.show', ['id' => $theme->id, 'theme_tab' => 'nav']) }}">{{ __('Navigation menu') }}</a>
    ...
    @foreach ($post_types_tabs as $post_type_tab_key => $post_type_tab_value)
        <a class="nav-item nav-link @if (($theme_tab ?? null) == 'contact') active @endif" href="{{ route('admin.themes.show', ['id' => $theme->id, 'theme_tab' => $post_type_tab_key]) }}">{{ $post_type_tab_value }}</a>
    @endforeach
</nav>
