<div class="row">
    ...
    <div class="col-lg-6">
        ...
        <div class="card bg-light p-3 mb-3">
            ...
            <form method="post" action="{{ route('admin.themes.update-template-part', ['slug' => $theme->slug, 'theme_tab' => 'nav', 'template_part' => 'nav']) }}">
                @csrf
                @method('PUT')

                @if (!($theme_config->theme_nav ?? null))
                    @include('template-parts.navigations.default.settings')
                @else
                    @include("template-parts.navigations.$theme_config->theme_nav.settings")
                @endif
