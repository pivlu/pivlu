        ...
        <div class="mt-2">
            <nav aria-label="breadcrumb" class="breadcrumb-header">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item" aria-current="page"><a href="{{ route('admin.templates.index') }}">{{ __('Templates') }}</a></li>
                    <li class="breadcrumb-item fw-bold" aria-current="page">{{ __('Pivlu Template Builder') }}</li>
                </ol>
            </nav>
        </div>

        <div class="mb-3">
            @include('admin.template-builder.includes.menu')
        </div>
                ...
                <div class="col-md-6 col-12">
                    ...
                    <div class="card bg-light p-3 mb-3">
                        ...
                        <div class="text-muted mb-3">
                            ...
                            <a target="_blank" href="{{ route('admin.template-builder.layouts') }}"><b>{{ __('Manage layouts') }}</b></a>
                        </div>
                    ...
                    <div class="card bg-light p-3 mb-3">
                        ...
                        <div class="form-group mb-0">
                            <div class="form-check form-switch">
                                <input type='hidden' value='' name='posts_custom_content'>
                                <input class="form-check-input" type="checkbox" id="posts_custom_content" name="posts_custom_content" @if ($config->posts_custom_content ?? null) checked @endif>
                                <label class="form-check-label" for="posts_custom_content">{{ __('Add a custom text in the posts main page') }}</label>
                            </div>
                            <div class="text-muted small">{{ __('Custom text is added at the top of the posts page') }}</div>
                        </div>

                        <script>
                            $('#posts_custom_content').change(function() {
                        ...
                        <div id="hidden_div_show_custom_text" style="display: @if ($config->posts_custom_content ?? null) block @else none @endif" class="mt-2">

                            <div class="form-group mb-3">
                                <div class="form-check form-switch">
                                    <input type='hidden' value='' name='posts_custom_content_on_categs'>
                                    <input class="form-check-input" type="checkbox" id="posts_custom_content_on_categs" name="posts_custom_content_on_categs" @if ($config->posts_custom_content_on_categs ?? null) checked @endif>
                                    <label class="form-check-label" for="posts_custom_content_on_categs">{{ __('Add this custom text in the posts categories pages too') }}</label>
                                </div>
                            </div>

                            @foreach ($posts_custom_title as $lang_posts_title)
                                <div class="row">
                                    <div class="col-12">
                                        <div class="form-group">
                                            <label>
                                                @if (count($languages) > 1)
                                                    {!! flag($lang_posts_title['lang']->code) !!}
                                                    @endif {{ __('Title') }} @if (count($languages) > 1)
                                                        -
                                                        {{ $lang_posts_title['lang']->name }} @if ($lang_posts_title['lang']->is_default)
                                                            ({{ __('default language') }})
                                                        @endif
                                                    @endif
                                            </label>
                                            <input type="text" name="posts_custom_title_{{ $lang_posts_title['lang']->id }}" class="form-control" value="{{ $lang_posts_title['content'] ?? null }}">
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                            ...
                            @foreach ($posts_custom_text as $lang_posts_text)
                                <div class="row">
                                    <div class="col-12">
                                        <div class="form-group">
                                            <label>
                                                @if (count($languages) > 1)
                                                    {!! flag($lang_posts_text['lang']->code) !!}
                                                    @endif {{ __('Text content') }} @if (count($languages) > 1)
                                                        -
                                                        {{ $lang_posts_text['lang']->name }} @if ($lang_posts_text['lang']->is_default)
                                                            ({{ __('default language') }})
                                                        @endif
                                                    @endif
                                            </label>
                                            <textarea name="posts_custom_text_{{ $lang_posts_text['lang']->id }}" class="form-control trumbowyg">{{ $lang_posts_text['content'] ?? null }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
