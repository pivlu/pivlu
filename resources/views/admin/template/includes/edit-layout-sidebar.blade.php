<div class="col-12">
    <div class="builder-col sortable" id="sortable_sidebar">
        <div class="mb-4 text-center">
            ...
            @include('admin.template.modals.add-layout-sidebar-block', ['layout' => 'sidebar'])
        </div>
        ...
        @foreach (layout_blocks($item->id, 'sidebar', $show_hidden = 1) as $block)
            <div class="builder-block movable" id="item-{{ $block->id }}">
                <div class="float-end ms-2">
                    ...
                    <a href="{{ route('admin.template.layouts.block', ['id' => $block->id]) }}" class="btn btn-primary btn-sm">{{ __('Manage content') }}</a>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    <form method="POST" action="{{ route('admin.template.layouts.content.delete', ['id' => $item->id, 'block_id' => $block->id]) }}">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
        ...
        $("#sortable_sidebar").sortable({
            ...
            update: function(event, ui) {
                ...
                $.ajax({
                    data: data,
                    type: 'POST',
                    url: "{{ route('admin.template.layouts.sortable', ['id' => $item->id, 'layout' => 'sidebar']) }}",
                });
            }
        });
        $("#sortable_sidebar").disableSelection();
