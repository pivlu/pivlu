<div class="form-group col-md-4 col-xl-3">
    <label>{{ __('Select slider') }}</label>
    @if (count($components_slider) == 0)
        <div class="text-danger mb-2">
            {{ __("You don't have any slider. Go to block compoents to create a new gallery") }}
            <br>
            <a target="_blank" href="{{ route('admin.block-components.type', ['type' => 'slider']) }}"><i class="bi bi-images"></i> {{ __('Add new slider') }}</a>
    ...
    @else
        ...
        <select class="form-select" name="slider_id">
            ...
            @foreach ($components_slider as $slider)
                <option @if (($block_settings->slider_id ?? null) == $slider->id) selected @endif value="{{ $slider->id }}">{{ $slider->label }}</option>
            @endforeach
        </select>
    @endif
</div>

<div class="form-group col-md-4 col-xl-2 col-xxl-1">
