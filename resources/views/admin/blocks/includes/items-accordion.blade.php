    ...
    <div class="card-header">
        <div class="row">
            <div class="col-12 col-sm-5 col-md-6 order-md-1 order-first">
                <h4 class="card-title">{{ __('Manage accordion items') }}</h4>
            </div>

            <div class="col-12 col-sm-7 col-md-6 order-md-2 order-last">
                <div class="float-end">
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#create-item"><i class="bi bi-plus-circle"></i> {{ __('Add new accordion item') }}</button>
                    @include('pivlu::admin.blocks.includes.modal-create-accordion-item')
                </div>
            </div>
        </div>
    ...
    <div class="card-body">
        ...
        <div class="sortable" id="sortable_top">
            @foreach ($block_items as $block_item)
                ...
                <div class="builder-block movable d-block" style="height: 110px" id="item-{{ $block_item['id'] ?? '-' }}">
                    ...
                    <div class="modal fade update-{{ $block_item['id'] ?? '-' }}" tabindex="-1" role="dialog" aria-labelledby="ConfirmUpdateLabel-{{ $block_item['id'] ?? '-' }}" aria-hidden="true">
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                ...
                                <form method="post" action="{{ route('admin.block.update-item', ['type' => 'accordion', 'block_id' => $block->id]) }}">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="modal-body">
                                        ...
                                        @foreach (admin_languages() as $item_lang)
                                            ...
                                            <div class="form-group">
                                                <label class="mb-2">{!! lang_label($item_lang, __('Content')) !!}</label>
                                                <textarea class="form-control trumbowyg" name="content_{{ $item_lang->id }}">{{ $block_item_data->content ?? null }}</textarea>
                                            </div>                                            
                    ...
                    <div class="line-clamp-1 mt-2 fw-bold">
                        {{ $block_item_content_data->title ?? null }}
                    </div>

                    <div class="line-clamp-2 mt-2">
                            {!! $block_item_content_data->content ?? null !!}
                        </div>
        ...
        $("#sortable_top").sortable({
            ...
            update: function(event, ui) {
                ...
                $.ajax({
                    data: data,
                    type: 'POST',
                    url: "{{ route('admin.block.sortable-items', ['type' => 'accordion', 'block_id' => $block->id]) }}",
                });
            }
        });
