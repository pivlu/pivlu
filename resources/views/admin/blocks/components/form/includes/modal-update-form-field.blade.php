...
<div class="modal fade custom-modal" tabindex="-1" role="dialog" aria-labelledby="updateLabel_{{ $field->id }}" aria-hidden="true" id="update-form-field-{{ $field->id }}">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form method="post" action="{{ route('admin.forms.config.update_field', ['id' => $block->id, 'field_id' => $field->id]) }}">
                @csrf
                @method('PUT')
                    ...
                    @foreach ($field->all_languages_contents as $field_content)
                        ...
                        <div id="hidden_div_dropdowns_{{ $field->id }}_{{ $field_content->lang_id }}" style="display: @if ($field->type == 'select' || $field->type == 'checkbox') block @else none @endif">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" rows="2" name="dropdowns_{{ $field_content->lang_id }}">{{ $field_content->dropdowns }}</textarea>
                            </div>
                        </div>
