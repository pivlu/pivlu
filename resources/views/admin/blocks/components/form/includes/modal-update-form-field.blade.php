        ...
        <div class="modal-content">
            ...
            <form method="post" action="{{ route('admin.forms.config.update_field', ['id' => $form->id, 'field_id' => $field->id]) }}">
                ...
                <div class="modal-body">
                    ...
                    @foreach (admin_languages() as $lang)
                        <h5 class="mb-3">@if (count(admin_languages()) > 1) {!! flag($lang->code) !!} {{ $lang->name }} @endif</h5>

                        <div class="form-group">
                            <label>{{ __('Label') }}</label>
                            {{--<input class="form-control" name="label_{{ $lang->id }}" value="{{ form_field_lang($field->id, $lang->id)->label ?? null }}" required>--}}
                            <input class="form-control" name="label_{{ $lang->id }}" value="" required>
                        </div>
                        ...
                        <div id="hidden_div_dropdowns_{{ $field->id }}_{{ $lang->id }}" style="display: @if($field->type == 'select' || $field->type == 'checkbox') block @else none @endif">
                            <div class="form-group">
                        ...
                        @if (count(admin_languages()) > 1 && !$loop->last)<hr>@endif
                    @endforeach
