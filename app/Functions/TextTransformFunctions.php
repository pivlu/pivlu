...
class TextTransformFunctions
{
    ...
    /**
     * Whether to keep headings in the output.
     */
    protected bool $keepTags = false;
    ...
    /**
     * Enable keeping h1 and h2 output.
     */
    public function keepTags(): static
    {
        $this->keepTags = true;
    ...
    protected function pipeline(): array
    {
        return array_merge(
            ...
            [
                ...
                fn(string $html) => strip_tags($html, $this->keepTags ? '<a>,<p>,<h1>,<h2>,<h3?,<b>,<strong>,<li>,<ol>,<ul>,<i>,<em>,<pre>,<code>' : null),

                // Strip all remaining HTML tags after purification.
                //fn(string $html) => strip_tags($html, $this->keepHeadings ? '<h1>' : null),
    ...
    protected function makePurifierConfig(): HTMLPurifier_Config
    {
        $allowed = array_filter([
            'p',  'h1', 'h2', 'h3', 'b', 'i', 'em',  'strong', 'ol', 'li', 'ul', 'pre', 'code', 'a[href]',            
            // 'p[style]',
        ]);
